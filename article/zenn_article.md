...
私たちのチームではmiroを使い、アイデア出しからプロジェクト管理までの一連のフローを可視化しながらプロジェクト進行を行いました。このやり方によって、現在どこまでプロジェクトが進捗していて、何をする必要があるのかが目で見てわかるため、スムーズなプロジェクト進行をすることができました。

### アイデアの策定と目標設定

最初のフェーズでは、チーム全員でアイデアを出し合いました。ハッカソンの制約（短期間での開発・限られたリソース）を考慮し、実現可能性の高いMVP（Minimum Viable Product、最小限の実用可能製品）のプランを検討しました。候補として以下のようなアイデアが挙がりました。（良いと思ったアイデアにスタンプが押されています。）
...
各メンバーが何ができるかやチームの課題を書き出し、各々がいつ作業できるかを洗い出して、プロジェクト提出期限までのスケジュールを立てました。メンバーのスキルセットの違いと作業できる時間の異なるため、このように課題を洗い出し、全員で解決策を考えることを容易にしたことで、チーム内での連携が容易になりました。
...
これらもmiroを使用して視覚的に表現できます。開発中には、進捗状況を確認し、未完成の部分をマークしたり、仕様説明を受ける際にも図を参照することで、視覚的に理解しやすいと感じました。
